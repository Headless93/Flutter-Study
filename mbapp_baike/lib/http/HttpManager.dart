...
import 'package:mbapp/utils/log_util.dart';
...
//      _client.interceptors..add(LcfarmLogInterceptor());
    ...
    try {
      ...
      if (statusCode == "20000") {
        ...
        if (successCallback != null) {
          successCallback(response.data["retval"]);
        }
      } else {
        //失败
  ...
  void _requestOld({
    ...
    try {
      ...
      String statusError = response.data["error"].toString();
      if(statusError != null && statusError!='null'){
        var jsonError= jsonDecode(statusError);
        String message = jsonError["message"];
        String code = jsonError["code"];
        LogUtil.v("请求服务器出错：$message");
        if (errorCallback != null) {
        ...
        //成功
        if (successCallback != null) {
          successCallback(jsonEncode(response.data));
        }
      }
  ...
  Future<T> _requestAsync<T>({
    ...
    try {
      ...
      if (statusCode == "0") {
        //成功
        if (jsonParse != null) {
          return jsonParse(response.data["retval"]);
        } else {
